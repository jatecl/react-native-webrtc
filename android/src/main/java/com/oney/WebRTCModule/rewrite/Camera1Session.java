...
import com.oney.WebRTCModule.LocalRecorder;
import org.webrtc.*;
...
public class Camera1Session implements CameraSession {
  ...
  private static final Histogram camera1ResolutionHistogram = Histogram.createEnumeration(
      "WebRTC.Android.Camera1.Resolution", COMMON_RESOLUTIONS.size());
  ...
  private LocalRecorder mediaRecorder = new LocalRecorder();
  ...
  public static void updateCameraParameters(android.hardware.Camera camera,
  ...
  public static CaptureFormat findClosestCaptureFormat(
  ...
  public static Size findClosestPictureSize(
  ...
  private void startCapturing() {
    ...
    CamcorderProfile pro = null;
    int[] formats = new int[]{CamcorderProfile.QUALITY_1080P, CamcorderProfile.QUALITY_720P, CamcorderProfile.QUALITY_480P, CamcorderProfile.QUALITY_CIF, CamcorderProfile.QUALITY_QVGA};
    for (int f : formats) {
      try {
        CamcorderProfile spro = CamcorderProfile.get(f);
        if (spro.videoFrameWidth < captureFormat.width || spro.videoFrameHeight < captureFormat.height) {
          if (pro == null) pro = spro;
          break;
        }
        pro = spro;
      }
    ...
    pro.videoFrameWidth = captureFormat.width;
    pro.videoFrameHeight = captureFormat.height;

    mediaRecorder.startRecord(applicationContext, camera, pro);
  }
  ...
  private int getFrameOrientation() {
    int lockOrientation = com.oney.WebRTCModule.LocalRecorder.Companion.getLockOrientation();
    int rotation;
    if (lockOrientation >= 0 && lockOrientation <= 3) {
